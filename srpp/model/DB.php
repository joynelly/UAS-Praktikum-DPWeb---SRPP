...
class DB{
  ...
  // WHERE but only once
  public function getWhereTwice($tableName, $condition, $condition2){
    $result = $this->getWhere2($tableName,$condition, $condition2);
    if (!empty($result)) {
      return $result[0];
    } else {
      return false;
  ...
  // WHERE condition
  public function getWhere2($tableName, $condition, $condition2){
    $queryCondition ="WHERE {$condition2[0]} {$condition2[1]} {$condition2[2]} AND {$condition[0]} {$condition[1]} ? ";
    return $this->get($tableName,$queryCondition,[$condition[2]]);
  }
  ...
  // INSERT SELECT Query
  public function insertSelect($tableName1, $tableName2, $data, $condition){
    $query = "INSERT INTO {$tableName1} SELECT ";
    foreach ($data as $key => $val){
      $query .= "$key = ?, " ;
    }
    $query = substr($query,0,-2);
    $query .= " FROM {$tableName2} WHERE {$condition[0]} {$condition[1]} ?";
